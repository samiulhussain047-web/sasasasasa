...
async login() {
  ...
  const url = `https://www.sparxmaths.uk/student/?s=${encodeURIComponent(this.schoolSlug)}`;
  console.log("Opened Chrome. Log in and click Homework.");
  ...
  page.on("response", async res => {
    ...
    if (headers.authorization && !foundToken) {
      foundToken = headers.authorization;
      console.log("✔ TOKEN");
    }

    if (headers["x-session-id"] && !foundSession) {
      foundSession = headers["x-session-id"];
      console.log("✔ SESSION");
    }
  });

  // wait until both exist
  while (!foundToken || !foundSession) {
...
_getCookies() {
  ...
  async getHomeworks() {
    ...
    const homeworkTasksResponse = await fetch(homeworkTasksRequestUrl, {
      method: "POST",
      headers: {
    ...
    if (homeworkTasksResponse.headers.get("grpc-status") !== null) {
      throw new Error(
        "Failed to get homeworks with status " +
          homeworkTasksResponse.headers.get("grpc-status")
      );
    }

    const homeworkTasks = await homeworkTasksResponse.blob();
    const completeDecodedProto = decodeProtoRecursive(
      Buffer.from(await homeworkTasks.arrayBuffer())
    );
    ...
    for (const sPackage of completeDecodedProto) {
      ...
      const homework = new Homework(
        ...
        new Date(
          parseInt(
            packageValue.find((value) => value.index === 2).value[0].value
          ) * 1000
        ),
        new Date(
          parseInt(
            packageValue.find((value) => value.index === 3).value[0].value
          ) * 1000
        ),
